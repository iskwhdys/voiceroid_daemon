    ...
    -c <filepath>"
		...
		private void StartServer() {
			...
			Task task = Task.Factory.StartNew(Run);
				...
				notify_icon.Text = $"{Config.VoiceName}:{Config.ListeningAddress}";
				notify_icon.Visible = true;

				// トレイアイコンのコンテキストメニューを作成する
				ContextMenu menu = new ContextMenu();
				menu.MenuItems.Add(new MenuItem("クリップボード読み上げ", new EventHandler((object sender, EventArgs e) => {
					if (Clipboard.ContainsText()) {
						Play(Clipboard.GetText());
					}
				})));
				menu.MenuItems.Add(new MenuItem("読み上げ停止", new EventHandler((object sender, EventArgs e) => {
					Stop();
				})));
				menu.MenuItems.Add(new MenuItem("クリップボードをクリアする", new EventHandler((object sender, EventArgs e) => {
					Clipboard.Clear();
				})));
		...
		private void Play(HttpListenerContext context) {
			HttpListenerRequest request = context.Request;
			HttpListenerResponse response = context.Response;
		...
		private void Play(string text) {
			...
			String kana = AitalkWrapper.TextToKana(text + "終わり。", Config.KanaTimeout);
			...
			Stop();
			player = new System.Media.SoundPlayer(new MemoryStream(buffer));
			player.Play();
		}

		private void SetResponse(HttpListenerResponse response) {
			...
			byte[] byte_data = Encoding.UTF8.GetBytes("fin");
			response.OutputStream.Write(byte_data, 0, byte_data.Length);
